...
- база данных `payment_gateway` в кодировке UTF-8;
- утилиты `curl` и `psql`.
...
Схема находится в `database/schema.sql`. Она создаёт таблицы `merchants` и
`payments`, внешний ключ, индекс и три начальные записи.

Из папки проекта выполните:

```sh
DB_PASSWORD='ваш_пароль' make setup-db
```

Команду можно запускать повторно: таблицы и начальные записи не дублируются.
